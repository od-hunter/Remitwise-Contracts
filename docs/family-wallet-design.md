...
| Sign transaction                   | `sign_transaction`                                           | `Member` or higher                                         | Must be in configured signer list for tx type; idempotent per signer (repeat calls are no-ops); proposer's implicit approval is not double-counted; non-member signer attempts return `SignerNotMember`. |
